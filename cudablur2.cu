...
int main(int argc,char** argv){
    ...
    host = (float *)malloc(sizeof(float)*pWidth*height);
    ...
    */
    ...
    cudaMemcpy(host, dest, pWidth*height*sizeof(float),cudaMemcpyDeviceToHost);
    for (i=0;i<pWidth*height;i++){
        img[i]=(uint8_t)host[i];
    }
    cudaFree(dest);   
